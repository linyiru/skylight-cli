...
function run(argv, { components = [['production', 'web'], ['staging', 'web']], env = {} } = {}) {
  const requests = [];
...
test('selects a component by environment/name and filters with Rails search', async () => {
  ...
  assert.deepEqual(result.endpoints.map(e => e.name), ['Admin::UsersController#index', 'UsersController#index']);
